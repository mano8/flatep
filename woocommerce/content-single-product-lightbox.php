...
<div class="row row-collapse mb-0 product" id="product-<?php the_ID(); ?>" <?php post_class(); ?>>
    ...
    <div class="slider slider-show-nav product-gallery-slider main-images mb-0">
      <?php
        if ( has_post_thumbnail() ) {
          ...
          echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<img src="%s" alt="%s" />', wc_placeholder_img_src( 'woocommerce_single' ), esc_html__( 'Awaiting product image', 'woocommerce' ) ), $post->ID );
  ...
  <div class="product-info summary large-6  col entry-summary" style="font-size:90%;">
    ...
    <a class="plain" href="<?php the_permalink();?>" <?php if(is_object($flatepWoo)) {echo $flatepWoo->get_the_product_link_title_seo($product);} ?>><h1><?php the_title(); ?></h1></a>
