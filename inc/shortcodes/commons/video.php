<?php if ( $video_mp4 || $video_webm || $video_ogg ) { 
    ...
    //-> To do 
    // - add new setting to activate skip video on mobile and another to add ipads
    // - set up poster image size
	// - understand how can't be poster an image type
	$vsrc = 'src';
	...
	<video class="video-bg fill <?php echo $video_visibility; ?>" preload playsinline autoplay
		...
		<?php
		echo $video_mp4 ? '<source ' . $vsrc .'="' . $video_mp4 . '" type="video/mp4">' : '';
		echo $video_ogg ? '<source ' . $vsrc .'="' . $video_ogg . '" type="video/ogg">' : '';
		echo $video_webm ? '<source ' . $vsrc .'="' . $video_webm . '" type="video/webm">' : '';
		?>
	</video>
<?php } ?>
<?php if ( $youtube ) { ?>
